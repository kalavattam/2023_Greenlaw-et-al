...
check_argument_secondary() {
    ...
    case "$(convert_chr_lower "${1}")" in
        ...
        false | f | 0) \
            secondary=0
            printf "%s\n" "\"Run in 'secondary' mode\" is FALSE."
            ;;
        *) \
...
main() {
    ...
    if [[ "${mode}" -eq 1 ]]; then
        ...
        if [[ "${unmapped}" -eq 1 ]]; then
            #  Including only (-f) unmapped reads (0x0004, 0x0008)
            samtools view \
                -@ "${threads}" \
                -f 0x0004 \
                -f 0x0008 \
        ...
        if [[ "${secondary}" -eq 1 ]]; then
            #  Including only (-f) secondary alignments (0x0100)
            samtools view \
                -@ "${threads}" \
                -f 0x0100 \
        ...
        #  Including only (-f) 0x0100, 0x0004, and 0x0008
        samtools view \
            -@ "${threads}" \
            -f 0x0100 \
            -f 0x0004 \
...
Arguments:
    ...
    -o  outdir     outfile directory, including path; if not found, will be
                   mkdir'd <chr>
    -v  unmapped   run in 'unmapped' mode <lgl> [default: TRUE]
    -s  secondary  run in 'secondary' mode <lgl> [default: TRUE]
