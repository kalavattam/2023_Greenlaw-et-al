...
This script takes as input Rcorrector error-corrected Illumina paired-end reads
in fastq format and

    1. Removes any reads that Rcorrector indentifes as containing an error but
       can't be corrected, typically low complexity sequences. For these, the
       header contains 'unfixable'.

    2. To avoid issues created by certain header formats for downstream tools,
       strips the ' cor' from headers of reads that Rcorrector fixed .

    3. Writes a log with counts of (a) read pairs that were removed because one
       end was unfixable, (b) corrected left and right reads, and (c) total
       number of read pairs containing at least one corrected read.

Currently, this script only handles paired-end reads; also, the scripts handles
