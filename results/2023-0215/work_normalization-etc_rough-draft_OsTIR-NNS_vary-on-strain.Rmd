...
#### Create rlog-normalized counts (unblinded)
...
#### Create GeTMM-normalized counts
...
#TODO Delete dgel? Or use it for DE analyses with edgeR?
```

### Run PCA
#### GeTMM-normalized counts
```{r}
draw_scree_plot <- function(pca, horn, elbow) {
    #TODO Descriptions for function and parameters
...
plot_biplot <- function(
    ...
    # loadings_show <- FALSE
    # loadings_n <- 0
    # meta_color <- "day_detailed_2"
    # meta_shape <- "cell_type"
    # x_min <- -150
    # x_max <- 150
    # y_min <- -100
    # y_max <- 100
    # title <- paste0(
    #     "PCA biplot: top ", n_loadings, " loadings, rlog-normalized\n",
    #     "RNA-seq counts for genes with row sums gte ",
    #     args$threshold_filter, " (PCAtools)"
    # )
...
plot_loadings <- function(x, y, z, a, b, d, e, f, g, h, i, j, k) {
    ...
    ggplot2::ggplot(x, ggplot2::aes(x = y, y = z)) +  #TODO #FUNCTION
...
horn_g <- PCAtools::parallelPCA(mat = norm_g[, c(1:5)])
obj_pca_g <- PCAtools::pca(
    norm_g[, c(1:5)],
    metadata = dds[dds@rowRanges$genome != "K_lactis", ]@colData
)
rownames(obj_pca_g$loadings) <- make.names(
    dds[dds@rowRanges$genome != "K_lactis", ]@rowRanges$feature,
    unique = TRUE
)
elbow_g <- PCAtools::findElbowPoint(obj_pca_g$variance)
scree_g <- draw_scree_plot(obj_pca_g, horn = horn_g$n, elbow = elbow_g)
scree_g

loadings <- as.data.frame(obj_pca_g$loadings)
PCs <- paste0("PC", 1:(as.numeric(elbow_g) + 2))
...
#  Analyze positive, negative loadings on axes (top 10 per each) of biplots ---
images <- list()
mat <- combn(PCs, 2)
for(i in 1:ncol(mat)) {
    # i <- 1
...
```{r}
horn_r <- PCAtools::parallelPCA(mat = norm_r[, c(1:5)])
obj_pca_r <- PCAtools::pca(
    norm_r[, c(1:5)],
    metadata = dds[dds@rowRanges$genome != "K_lactis", ]@colData
)
rownames(obj_pca_r$loadings) <- make.names(
    dds[dds@rowRanges$genome != "K_lactis", ]@rowRanges$feature,
    unique = TRUE
)
elbow_r <- PCAtools::findElbowPoint(obj_pca_r$variance)
scree_r <- draw_scree_plot(obj_pca_r, horn = horn_r$n, elbow = elbow_r)
scree_r
```
