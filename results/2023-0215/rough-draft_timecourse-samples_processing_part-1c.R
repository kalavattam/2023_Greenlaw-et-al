...
#  rough-draft_timecourse-samples_processing_part-1c.R
#  KA
...
if(filtering == "none"){
    ...
    counts <- sapply(t_tc[t_tc$genome == "S_cerevisiae", 12:ncol(t_tc)], as.numeric)
    keep <- rowSums(counts >= 10) >= 3
    t_tmp <- t_tc[t_tc$genome == "S_cerevisiae", ]
    t_tmp <- t_tmp[keep, ]  # 10749
...
t_tc <- dplyr::bind_rows(t_tmp, t_tc[t_tc$genome != "S_cerevisiae", ])
