...
## ...
### ...
Having written and run the code in `work_calculate_uni-multimappers-etc.md`...
...
#  Read in the dataframe for `work_calculate_uni-multimappers-etc.md` metrics
...
#  Run the processing steps detailed above ------------------------------------
...
#  Step 7: Get numbers of all counts associated for "SC-I-XVI"
`SC-I-XVI_all` <- 
...
ovation <- dplyr::bind_rows(
    ovation, `__alignment_not_unique_I-XVI`, `__no_feature_I-XVI`,
    `__sum_I-XVI`
)
...
df_tmp_1 <- dplyr::bind_cols(
    ovation[1:(nrow(ovation) - 3), ],
    comp_S,
    by = "gene_id"
) %>%
    dplyr::select(-c(gene_id...17, by)) %>%
    dplyr::rename(gene_id = gene_id...1)
df_tmp_2 <- ovation[(nrow(ovation) - 2):nrow(ovation), ]

test <- dplyr::bind_rows(df_tmp_1, df_tmp_2)
test$category <- ifelse(is.na(test$category), test$gene_id, test$category)
test_summarize <- test %>%
    dplyr::group_by(category) %>%
    dplyr::summarize(
...
test_summarize$category[stringr::str_detect(
    test_summarize$category, "__alignment_not_unique_I-XVI"
)] <- "multimapper (excluded)"
test_summarize$category[stringr::str_detect(
    test_summarize$category, "__ambiguous"
)] <- "ambiguous"
test_summarize$category[stringr::str_detect(
    test_summarize$category, "__no_feature_I-XVI"
)] <- "no feature"
test_summarize$category[stringr::str_detect(
    test_summarize$category, "PG"
)] <- "pseudogene"
...
test_summarize$number_of_features <- ifelse(
    test_summarize$number_of_features == 1,
    NA_integer_,
    test_summarize$number_of_features
)

test_summarize_p <- test_summarize %>%
...
test_summarize_p$sample <- stringr::str_remove(test_summarize_p$sample, "sum_WT_")

# test_summarize_t <- test_summarize[, -ncol(test_summarize)] %>%
#     t() %>%
#     as.data.frame() %>%
#     tibble::rownames_to_column()
# colnames(test_summarize_t) <- test_summarize_t[1, ]
# test_summarize_t <- test_summarize_t[-1, ]
...
test_summarize_p %>%
    ...
    geom_bar(position = "fill", stat = "identity") +
    scale_fill_manual(
        values = length(test_summarize$category) %>%
            viridisLite::viridis() %>%
            sample()
    ) +
    theme_slick
...
#  Try with means
test_summarize$mean_WT_G1_N <- apply(test_summarize[, 2:3], 1, function(x) mean(x))
test_summarize$mean_WT_G1_SS <- apply(test_summarize[, 4:5], 1, function(x) mean(x))
test_summarize$mean_WT_Q_N <- apply(test_summarize[, 6:7], 1, function(x) mean(x))
test_summarize$mean_WT_Q_SS <- apply(test_summarize[, 8:9], 1, function(x) mean(x))

test_summarize$SEM_WT_G1_N <- apply(test_summarize[, 2:3], 1, function(x) sd(x)/sqrt(length(x)))
test_summarize$SEM_WT_G1_SS <- apply(test_summarize[, 4:5], 1, function(x) sd(x)/sqrt(length(x)))
test_summarize$SEM_WT_Q_N <- apply(test_summarize[, 6:7], 1, function(x) sd(x)/sqrt(length(x)))
test_summarize$SEM_WT_Q_SS <- apply(test_summarize[, 8:9], 1, function(x) sd(x)/sqrt(length(x)))
...
test_summarize_p <- test_summarize %>%
    ...
    dplyr::rename(sample = name, counts = value)
test_summarize_p$sample <- stringr::str_remove(test_summarize_p$sample, "mean_WT_")
...
test_summarize_p %>%
    ...
    theme_slick
# rm(test, test_summarize)

```
</details>
<br />

~~So, what are the summary features we need to retain in dataframe `ovation`?~~ *Done.*
