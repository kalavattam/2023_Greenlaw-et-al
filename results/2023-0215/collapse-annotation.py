...
from bx.intervals.intersection import IntervalTree  # In Trinity_env, did mamba install -c bioconda bx-python (2023-0405)
import argparse
import os
import gzip
...
class Transcript:
    # def __init__(self, transcript_id, transcript_name, transcript_type, gene, start_pos, end_pos):  #CHANGED
    # def __init__(self, transcript_id, transcript_name, gene, start_pos, end_pos):  #CHANGED
    # def __init__(self, transcript_id, transcript_name, start_pos, end_pos):  #CHANGED
    # def __init__(self, transcript_id, start_pos, end_pos):
    def __init__(self, transcript_id, gene_id, chrom, strand, start_pos, end_pos):
        self.id = transcript_id
        self.id = gene_id  #ADDITION
        # self.name = transcript_name  #CHANGED
        # self.type = transcript_type  #CHANGED
        # self.gene = gene  #CHANGED
        self.chr = chrom
        self.strand = strand
        self.start_pos = start_pos
        self.end_pos = end_pos
...
class Annotation:
    def __init__(self, gtfpath):
        ...
        # gtfpath = args.transcript_gtf  #TESTINGONLY
            ...
            for row in gtf:
                ...
                if annot_type == 'gene':
                    ...
                    t = Transcript(
                        ...
                        # attributes.pop('transcript_name'),  #CHANGED
                        # attributes.pop('transcript_type'),  #CHANGED
                        # g,  #CHANGED
                        chrom,  #ADDITION
                        strand,  #ADDITION
                        start_pos,
                    ...
                    t.source = row[1]
                    t.phase = row[7]
                    t.attributes_string = row[8].replace('_biotype', '_type')
                    t.attributes = attributes
                    # g.transcripts.append(t)  #CHANGED
                    self.transcripts.append(t)  #ADDITION
                ...
                if len(self.transcripts) % 1000 == 0:
                    print(f'\rParsing GTF: {len(self.transcripts)} transcripts processed', end='')  #ADDITION
            print(f'\rParsing GTF: {len(self.transcripts)} transcripts processed')  #ADDITION

        self.transcripts = np.array(self.transcripts)  #ADDITION

                # if len(self.genes) % 1000 == 0:  #CHANGED
                #     print(f'\rParsing GTF: {len(self.genes)} genes processed', end='')
            # print(f'\rParsing GTF: {len(self.genes)} genes processed')  #CHANGED

        # self.genes = np.array(self.genes)  #CHANGED
...
def collapse_annotation(
    ...
    annot = annotation  #TESTINGONLY
    transcript_gtf=args.transcript_gtf  #TESTINGONLY
    collapsed_gtf=args.output_gtf  #TESTINGONLY
    blacklist=blacklist  #TESTINGONLY
    collapse_only=False  #TESTINGONLY
    stranded=True  #TESTINGONLY

    exclude = set(['retained_intron', 'readthrough_transcript'])

    # 1) collapse each transcript, excluding blacklisted transcript types
    merged_coord_dict = {}
    for t in annot.transcripts:
        exon_coords = []
        if (
            (t.id not in blacklist) and
            (
                ('tags' not in t.attributes) or
                len(set(t.attributes['tags']).intersection(exclude)) == 0
            )
        ):
            for e in t.exons:
                exon_coords.append([e.start_pos, e.end_pos])
    if exon_coords:
        merged_coord_dict[t.id] = interval_union(exon_coords)

    if not collapse_only:
        # 2) build interval tree with merged domains
        interval_trees = defaultdict(IntervalTree)
        for t in annot.transcripts:
            if t.id in merged_coord_dict:
                for i in merged_coord_dict[t.id]:
                    # half-open intervals [a,b)
                    if stranded:
                        interval_trees[t.chr, t.strand].add(i[0], i[1]+1, [i, t.id])
                    else:
                        interval_trees[t.chr].add(i[0], i[1]+1, [i, t.id])

        # 3) query intervals of each transcript, remove overlaps
        new_coord_dict = {}
        for t in annot.transcripts:
            if t.id in merged_coord_dict:
                new_intervals = []
                for i in merged_coord_dict[t.id]:  # loop merged exons
                    if stranded:
                        ints = interval_trees[t.chr, t.strand].find(i[0], i[1]+1)
                    else:
                        ints = interval_trees[t.chr].find(i[0], i[1]+1)
                    # remove self
                    ints = [r[0] for r in ints if r[1] != t.id]
                    m = set([tuple(i)])
                    for v in ints:
                        m = [subtract_segment(mx, v) for mx in m]
                        # flatten
                    ...
                    new_intervals.extend(m)
                if new_intervals:
                    new_coord_dict[t.id] = new_intervals

        # 4) remove transcripts containing single-base exons only
        for t in annot.transcripts:
            if t.id in new_coord_dict:
                exon_lengths = np.array([i[1]-i[0]+1 for i in new_coord_dict[t.id]])
                if np.all(exon_lengths == 1):
                    new_coord_dict.pop(t.id)
    else:
        new_coord_dict = merged_coord_dict
    ...
    with open(collapsed_gtf, 'w') as output_gtf, opener as input_gtf:
        ...
        for line in input_gtf:
            if line[0] == '#':
                ...
                break
        output_gtf.write('##collapsed version generated by modified GTEx pipeline\n')  #ADDITION
        for t in annot.transcripts:
            if t.id in new_coord_dict:
                start_pos = str(np.min([i[0] for i in new_coord_dict[t.id]]))
                end_pos = str(np.max([i[1] for i in new_coord_dict[t.id]]))
                if 'transcript_id' in t.attributes_string:
                    attr = t.attributes_string
                else:
                    attr = add_transcript_attributes(t.attributes_string)
                output_gtf.write('\t'.join([t.chr, t.source, 'gene', start_pos, end_pos, '.', t.strand, t.phase, attr])+'\n')
                output_gtf.write('\t'.join([t.chr, t.source, 'transcript', start_pos, end_pos, '.', t.strand, t.phase, attr])+'\n')
                if t.strand == '-':
                    new_coord_dict[t.id] = new_coord_dict[t.id][::-1]
                for k,i in enumerate(new_coord_dict[t.id], 1):
                    output_gtf.write('\t'.join([
                        t.chr, t.source, 'exon', str(i[0]), str(i[1]), '.', t.strand, t.phase,
                        attr+f' exon_id "{t.id}_{k}; exon_number {k}";'])+'\n')


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description=(
            'Collapse isoforms into single transcript per gene '
            'and remove overlapping intervals between genes'
    ))
    parser.add_argument(
        'transcript_gtf',
        help='Transcript annotation in GTF format'
    )
    parser.add_argument(
        'output_gtf',
    ...
    # code.activestate.com/recipes/579103-python-addset-attributes-to-list/
    class L(list):
        """
    ...
    args = L()
    args.transcript_gtf = 'Q_mkc-1_gte-pctl-40.gtf'
    # args.transcript_gtf = 'Q_mkc-1_gte-pctl-40.ordered.gtf'
    args.output_gtf = 'test.gtf'
    args.transcript_blacklist = ''
    args.collapse_only = False
    args.stranded = True
