...
#         for(k in f_part_5) {
            ...
            counts_relevant_dds <- DESeq2::DESeqDataSetFromMatrix(
                countData = counts_relevant[, 2:ncol(counts_relevant)],
                colData = metadata,
                design = ~ 1,
                rowRanges = pos_info
            )
            ...
            #  - Step 0: Combine feature counts with htseq-count-/samtools-
            #+           derived summary values
            combined_raw <- dplyr::bind_rows(
                counts_relevant, summary_values, uni_multi_etc_rel
            )
            
            #  Run processing steps for combined feature/summary counts
            #  ------------------------
            ...
            #  Order the categories alphabetically without respect to case, and
            #+ exclude the "multimapper (excluded)" category
            summary <- summary %>%
                dplyr::arrange(tolower(category)) %>%
                dplyr::filter(!stringr::str_detect(
                    category, "^multimapper*"
            ...
            #  Remove unnecessary string "sum_" from the column names
            colnames(summary) <- colnames(summary)
            
            #  Create a version of dataframe summary for proportions
            summary_prop <- summary[, 2:9] %>%
                prop.table() %>%
                dplyr::mutate(
            ...
            summary_prop.pre <- summary_prop
            # summary_prop <- summary_prop.pre
            
            summary_prop$mean_G1_N <- apply(
                summary_prop.pre[, 2:3], 1, function(x) mean(x)
            )
            summary_prop$SEM_G1_N <- apply(
                summary_prop.pre[, 2:3], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary_prop$mean_G1_SS <- apply(
                summary_prop.pre[, 4:5], 1, function(x) mean(x)
            )
            summary_prop$SEM_G1_SS <- apply(
                summary_prop.pre[, 4:5], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary_prop$mean_Q_N <- apply(
                summary_prop.pre[, 6:7], 1, function(x) mean(x)
            )
            summary_prop$SEM_Q_N <- apply(
                summary_prop.pre[, 6:7], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary_prop$mean_Q_SS <- apply(
                summary_prop.pre[, 8:9], 1, function(x) mean(x)
            )
            summary_prop$SEM_Q_SS <- apply(
                summary_prop.pre[, 8:9], 1, function(x) sd(x) / sqrt(length(x))
            )
            ...
            #  Prepare "pivoted" dataframe from summary_prop mean, SEM, etc.
            df_tmp_1 <- summary_prop %>%
                dplyr::select(-c(
                    G1_N_rep1, G1_N_rep2, G1_SS_rep1, G1_SS_rep2,
                    Q_N_rep1, Q_N_rep2, Q_SS_rep1, Q_SS_rep2,
                    SEM_G1_N, SEM_G1_SS, SEM_Q_N, SEM_Q_SS,
            ...
            #  Save the pivoted dataframe to variable
            summary_prop_mean_SEM <- dplyr::bind_cols(
                df_tmp_1[, 1:2], df_tmp_2[, 2:3]
            )
