...
`#NOTE` Functions and themes `theme_slick`, `theme_slick_no_legend`, `%notin%`, `draw_scree_plot`, `get_name_of_var`, `get_top_loadings`, `plot_biplot`, `plot_loadings`, `plot_pos_neg_loadings_each_axis`, `read_in_counts_matrix`, and `record_positional_info` are already loaded into the environment. Chunk `Get situated` has been run as well.
...
relevant <- c(
    "bams_renamed/UT_prim_UMI/WT_G1_day1_ovn_N_aux-F_tc-F_rep1_tech1.UT_prim_UMI.bam",
    "bams_renamed/UT_prim_UMI/WT_G1_day1_ovn_N_aux-F_tc-F_rep2_tech1.UT_prim_UMI.bam",
    "bams_renamed/UT_prim_UMI/WT_G1_day1_ovn_SS_aux-F_tc-F_rep1_tech1.UT_prim_UMI.bam",
    "bams_renamed/UT_prim_UMI/WT_G1_day1_ovn_SS_aux-F_tc-F_rep2_tech1.UT_prim_UMI.bam",
    "bams_renamed/UT_prim_UMI/WT_Q_day7_ovn_N_aux-F_tc-F_rep1_tech1.UT_prim_UMI.bam",
    "bams_renamed/UT_prim_UMI/WT_Q_day7_ovn_N_aux-F_tc-F_rep2_tech1.UT_prim_UMI.bam",
    "bams_renamed/UT_prim_UMI/WT_Q_day7_ovn_SS_aux-F_tc-F_rep1_tech1.UT_prim_UMI.bam",
    "bams_renamed/UT_prim_UMI/WT_Q_day7_ovn_SS_aux-F_tc-F_rep2_tech1.UT_prim_UMI.bam"
)
...
for(i in f_part_3) {
    for(j in f_part_4) {
        for(k in f_part_5) {
            ...
            relevant <- colnames(counts)[stringr::str_detect(colnames(counts), "ovn")]
            
            #  ----------------------------------------------------------------
            #  Isolate relevant WT G1 and Q data: htseq-count counts
            counts_relevant <- counts[, c("gene_id", relevant)] %>%
                dplyr::filter(!stringr::str_detect(gene_id, "^__"))
            colnames(counts_relevant) <- colnames(counts_relevant) %>%
                gsub("bams_renamed/UT_prim_UMI/", "", .) %>%
                gsub("\\.UT_prim_UMI\\.bam", "", .) %>%
                gsub("*_day._ovn*", "", .) %>%
                gsub("aux-F_tc-F_", "", .) %>%
            ...
            summary_values <- counts[, c("gene_id", relevant)] %>%
                dplyr::filter(stringr::str_detect(gene_id, "^__"))
            colnames(summary_values) <- colnames(summary_values) %>%
                gsub("bams_renamed/UT_prim_UMI/", "", .) %>%
                gsub("\\.UT_prim_UMI\\.bam", "", .) %>%
                gsub("*_day._ovn*", "", .) %>%
                gsub("aux-F_tc-F_", "", .) %>%
            ...
            metadata <- counts_relevant[, 2:ncol(counts_relevant)] %>%
                ...
                t() %>%
                as.data.frame()
            rownames(metadata) <- counts_relevant[, 2:ncol(counts_relevant)] %>%
                colnames()
            colnames(metadata) <- c("state", "transcription", "replicate")
            ...
            #  Test replicate consistency via PCA of non-normalized counts
            #  Create a PCAtools "pca" S4 object for the raw counts
            raw_pca <- PCAtools::pca(
                counts_relevant[, 2:ncol(counts_relevant)],
                metadata = metadata
            )
            rownames(raw_pca$loadings) <- dplyr::pull(counts_relevant, gene_id)
            
            #  Determine "significant" PCs with Horn's parallel analysis (see
            #+ Horn, 1965)
            raw_horn <- PCAtools::parallelPCA(
                counts_relevant[, 2:ncol(counts_relevant)]
            )
            
            #  Determine "significant" principle components with the elbow
            #+ method (see Buja and Eyuboglu, 1992)
            raw_elbow <- PCAtools::findElbowPoint(raw_pca$variance)
            
            #  Evaluate cumulative proportion of explained variance with a
            #+ scree plot
            raw_scree <- draw_scree_plot(
                raw_pca, horn = raw_horn$n, elbow = raw_elbow
            )
            
            #  Save component loading vectors in their own dataframe
            raw_loadings <- as.data.frame(raw_pca$loadings)
            
            #  Evaluate the component loading vectors for the number of
            #+ "significant" PCs identified via the elbow method plus two
            raw_PCs <- paste0("PC", 1:(as.numeric(raw_elbow) + 2))
            raw_top_loadings_all <- lapply(
                raw_PCs, get_top_loadings, x = raw_loadings, z = "all", a = TRUE
            )
            raw_top_loadings_pos <- lapply(
                raw_PCs, get_top_loadings, x = raw_loadings, z = "pos", a = TRUE
            )
            raw_top_loadings_neg <- lapply(
                raw_PCs, get_top_loadings, x = raw_loadings, z = "neg", a = TRUE
            )
            
            names(raw_top_loadings_all) <-
                names(raw_top_loadings_pos) <-
                names(raw_top_loadings_neg) <-
                raw_PCs
            
            #  Evaluate positive and negative loadings on axes of biplots
            #+ Look at the top 15 per axis
            raw_images <- list()
            raw_mat <- combn(raw_PCs, 2)
            for(l in 1:ncol(raw_mat)) {
                # l <- 1
                m<- raw_mat[, l]
                ...
                raw_images[[paste0("PCAtools.", PC_x, ".v.", PC_y)]] <-
                    plot_biplot(
                        pca = raw_pca,
                        PC_x = PC_x,
                        PC_y = PC_y,
                ...
                raw_images[[paste0("KA.", PC_x, ".v.", PC_y)]] <-
                    plot_pos_neg_loadings_each_axis(
                        df_all = raw_loadings,
                        df_pos = raw_top_loadings_pos,
                        df_neg = raw_top_loadings_neg,
                        PC_x = PC_x,
                        PC_y = PC_y,
                        row_start = 1,
                        row_end = 15,  # 30
            ...
            matrices[[name]][["08_raw_pca"]] <- raw_pca
            matrices[[name]][["09_raw_horn"]] <- raw_horn
            matrices[[name]][["10_raw_elbow"]] <- raw_elbow
            matrices[[name]][["11_raw_scree"]] <- raw_scree
            matrices[[name]][["12_raw_loadings"]] <- raw_loadings
            matrices[[name]][["13_raw_PCs"]] <- raw_PCs
            matrices[[name]][["14_raw_top_loadings_all"]] <- raw_top_loadings_all
            matrices[[name]][["15_raw_top_loadings_pos"]] <- raw_top_loadings_pos
            matrices[[name]][["16_raw_top_loadings_neg"]] <- raw_top_loadings_neg
            matrices[[name]][["17_raw_images"]] <- raw_images
            matrices[[name]][["18_raw_mat"]] <- raw_mat
