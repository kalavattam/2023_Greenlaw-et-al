---
title: "rough-draft_evaluate-categories_expression.Rmd"
...
ovation_summarize %>%
    ...
    dplyr::rename(sample = name, counts = value) %>%
    ggplot(aes(fill = category, y = counts, x = sample)) +
    geom_bar(position = "fill", stat = "identity") +
    scale_fill_manual(
...
#HERE
`plot-rep-prop_by-sample_stacked` <- function(x) {
    # ...
    # :param x: Pivoted summary tibble
    # :return y: ...
    y <- x %>%
        ggplot(aes(x = sample, y = counts, fill = category)) +
            geom_bar(position = "fill", stat = "identity") +
            theme_slick +
...
`plot-rep-prop_by-category_stacked` <- function(x) {
    # ...
    # :param x: Pivoted summary tibble
    # :return y: ...
    y <- x %>%
        ggplot(aes(x = category, y = counts, fill = sample)) +
            geom_bar(position = "fill", stat = "identity") +
            theme_slick +
            ggpubr::rotate_x_text(45) +
            ggtitle(
...
`plot-rep-prop_by-category_dodged` <- function(x) {
    # ...
    # :param x: Pivoted summary tibble
    # :return y: ...
    y <- x %>%
        ggplot(aes(
            x = category, y = counts / sum(counts), fill = sample
        )) +
            geom_bar(position = "dodge", stat = "identity") +
            theme_slick +
            ggpubr::rotate_x_text(45) +
...
`plot-rep-prop_by-sample_dodged` <- function(x) {
    # ...
    # :param x: Pivoted summary tibble
    # :return y: ...
    y <- x %>%
        ggplot(aes(
            x = sample, y = counts / sum(counts), fill = category
        )) +
            geom_bar(position = "dodge", stat = "identity") +
            theme_slick +
            ggpubr::rotate_x_text(45) +
...
#         for(k in f_part_5) {
            ...
            combined_raw <- dplyr::bind_rows(
                counts_relevant, summary_values, uni_multi_etc_rel
            )
...
```{r}
            ...
            summary %>%
                ...
                `plot-rep-prop_by-sample_stacked`() +
                scale_fill_manual(
                    values = viridisLite::viridis(length(summary$category))
                )
...
```{r}
            ...
            summary$mean_G1_N <- apply(
                summary.pre[, 2:3], 1, function(x) mean(x)
            )
            summary$SEM_G1_N <- apply(
                summary.pre[, 2:3], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary$mean_G1_SS <- apply(
                summary.pre[, 4:5], 1, function(x) mean(x)
            )
            summary$SEM_G1_SS <- apply(
                summary.pre[, 4:5], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary$mean_Q_N <- apply(
                summary.pre[, 6:7], 1, function(x) mean(x)
            )
            summary$SEM_Q_N <- apply(
                summary.pre[, 6:7], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary$mean_Q_SS <- apply(
                summary.pre[, 8:9], 1, function(x) mean(x)
            )
            summary$SEM_Q_SS <- apply(
                summary.pre[, 8:9], 1, function(x) sd(x) / sqrt(length(x))
            )
            ...
            prop_summary$mean_G1_N <- apply(
                prop_summary.pre[, 2:3], 1, function(x) mean(x)
            )
            prop_summary$SEM_G1_N <- apply(
                prop_summary.pre[, 2:3], 1, function(x) sd(x) / sqrt(length(x))
            )
            prop_summary$mean_G1_SS <- apply(
                prop_summary.pre[, 4:5], 1, function(x) mean(x)
            )
            prop_summary$SEM_G1_SS <- apply(
                prop_summary.pre[, 4:5], 1, function(x) sd(x) / sqrt(length(x))
            )
            prop_summary$mean_Q_N <- apply(
                prop_summary.pre[, 6:7], 1, function(x) mean(x)
            )
            prop_summary$SEM_Q_N <- apply(
                prop_summary.pre[, 6:7], 1, function(x) sd(x) / sqrt(length(x))
            )
            prop_summary$mean_Q_SS <- apply(
                prop_summary.pre[, 8:9], 1, function(x) mean(x)
            )
            prop_summary$SEM_Q_SS <- apply(
                prop_summary.pre[, 8:9], 1, function(x) sd(x) / sqrt(length(x))
            )
            ...
            columnwise_prop_summary$mean_G1_N <- apply(
                columnwise_prop_summary.pre[, 2:3], 1, function(x) mean(x)
            )
            columnwise_prop_summary$SEM_G1_N <- apply(
                columnwise_prop_summary.pre[, 2:3], 1, function(x) sd(x) / sqrt(length(x))
            )
            columnwise_prop_summary$mean_G1_SS <- apply(
                columnwise_prop_summary.pre[, 4:5], 1, function(x) mean(x)
            )
            columnwise_prop_summary$SEM_G1_SS <- apply(
                columnwise_prop_summary.pre[, 4:5], 1, function(x) sd(x) / sqrt(length(x))
            )
            columnwise_prop_summary$mean_Q_N <- apply(
                columnwise_prop_summary.pre[, 6:7], 1, function(x) mean(x)
            )
            columnwise_prop_summary$SEM_Q_N <- apply(
                columnwise_prop_summary.pre[, 6:7], 1, function(x) sd(x) / sqrt(length(x))
            )
            columnwise_prop_summary$mean_Q_SS <- apply(
                columnwise_prop_summary.pre[, 8:9], 1, function(x) mean(x)
            )
            columnwise_prop_summary$SEM_Q_SS <- apply(
                columnwise_prop_summary.pre[, 8:9], 1, function(x) sd(x) / sqrt(length(x))
            )
            ...
            summary_other$mean_G1_N <- apply(
                summary_other.pre[, 2:3], 1, function(x) mean(x)
            )
            summary_other$SEM_G1_N <- apply(
                summary_other.pre[, 2:3], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary_other$mean_G1_SS <- apply(
                summary_other.pre[, 4:5], 1, function(x) mean(x)
            )
            summary_other$SEM_G1_SS <- apply(
                summary_other.pre[, 4:5], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary_other$mean_Q_N <- apply(
                summary_other.pre[, 6:7], 1, function(x) mean(x)
            )
            summary_other$SEM_Q_N <- apply(
                summary_other.pre[, 6:7], 1, function(x) sd(x) / sqrt(length(x))
            )
            summary_other$mean_Q_SS <- apply(
                summary_other.pre[, 8:9], 1, function(x) mean(x)
            )
            summary_other$SEM_Q_SS <- apply(
                summary_other.pre[, 8:9], 1, function(x) sd(x) / sqrt(length(x))
            )
            ...
            prop_summary_other$mean_G1_N <- apply(
                prop_summary_other.pre[, 2:3], 1, function(x) mean(x)
            )
            prop_summary_other$SEM_G1_N <- apply(
                prop_summary_other.pre[, 2:3], 1, function(x) sd(x) / sqrt(length(x))
            )
            prop_summary_other$mean_G1_SS <- apply(
                prop_summary_other.pre[, 4:5], 1, function(x) mean(x)
            )
            prop_summary_other$SEM_G1_SS <- apply(
                prop_summary_other.pre[, 4:5], 1, function(x) sd(x) / sqrt(length(x))
            )
            prop_summary_other$mean_Q_N <- apply(
                prop_summary_other.pre[, 6:7], 1, function(x) mean(x)
            )
            prop_summary_other$SEM_Q_N <- apply(
                prop_summary_other.pre[, 6:7], 1, function(x) sd(x) / sqrt(length(x))
            )
            prop_summary_other$mean_Q_SS <- apply(
                prop_summary_other.pre[, 8:9], 1, function(x) mean(x)
            )
            prop_summary_other$SEM_Q_SS <- apply(
                prop_summary_other.pre[, 8:9], 1, function(x) sd(x) / sqrt(length(x))
            )
            ...
            columnwise_prop_summary_other$mean_G1_N <- apply(
                columnwise_prop_summary_other.pre[, 2:3], 1, function(x) mean(x)
            )
            columnwise_prop_summary_other$SEM_G1_N <- apply(
                columnwise_prop_summary_other.pre[, 2:3], 1, function(x) sd(x) / sqrt(length(x))
            )
            columnwise_prop_summary_other$mean_G1_SS <- apply(
                columnwise_prop_summary_other.pre[, 4:5], 1, function(x) mean(x)
            )
            columnwise_prop_summary_other$SEM_G1_SS <- apply(
                columnwise_prop_summary_other.pre[, 4:5], 1, function(x) sd(x) / sqrt(length(x))
            )
            columnwise_prop_summary_other$mean_Q_N <- apply(
                columnwise_prop_summary_other.pre[, 6:7], 1, function(x) mean(x)
            )
            columnwise_prop_summary_other$SEM_Q_N <- apply(
                columnwise_prop_summary_other.pre[, 6:7], 1, function(x) sd(x) / sqrt(length(x))
            )
            columnwise_prop_summary_other$mean_Q_SS <- apply(
                columnwise_prop_summary_other.pre[, 8:9], 1, function(x) mean(x)
            )
            columnwise_prop_summary_other$SEM_Q_SS <- apply(
                columnwise_prop_summary_other.pre[, 8:9], 1, function(x) sd(x) / sqrt(length(x))
            )
...
```{r}
            ...
            summary_other_mean_SEM %>%
                ...
                `plot-rep-prop_by-sample_stacked`() +
                scale_fill_manual(
                    values = viridisLite::viridis(length(summary_other$category))
                )
...
```{r}
            ...
            prop_summary_other_mean_SEM %>%
                group_by(category) %>%
                ggplot(aes(x = sample, y = mean, fill = category)) +
                geom_bar(position = "dodge", stat = "identity") +
                geom_errorbar(
            ...
            prop_summary_other_mean_SEM %>%
                group_by(category) %>%
                ggplot(aes(x = category, y = mean, fill = sample)) +
                geom_bar(position = "dodge", stat = "identity") +
                geom_errorbar(
                    aes(ymin = mean - SEM, ymax = mean + SEM),
                    width = 0.66,
...
res.stats <- df %>%
    dplyr::group_by(dose) %>%
    rstatix::t_test(len ~ supp, alternative = "two.sided", var.equal = TRUE) %>%
...
p + ggpubr::stat_pvalue_manual(
    res.stats, x = "dose", y.position = c(30, 45, 60), label = "p.adj.signif"
)


#  Trying the above with my data ----------------------------------------------
df
a <- columnwise_prop_summary_other[, 1:9] %>%
    tidyr::pivot_longer(cols = c(
        G1_N_rep1, G1_N_rep2, G1_SS_rep1, G1_SS_rep2,
        Q_N_rep1, Q_N_rep2, Q_SS_rep1, Q_SS_rep2
    ))
a$name <- a$name %>% stringr::str_remove("_rep1|_rep2") %>% factor()

a.res.stats <- a %>%
    dplyr::group_by(category) %>%
    rstatix::t_test(value ~ name, alternative = "two.sided", var.equal = TRUE) %>%
    rstatix::add_significance()

a.p <- ggpubr::ggbarplot(
    a, x = "name", y = "value", color = "black", fill = "category", palette = viridisLite::viridis(7), label = FALSE, add = "mean_se"#, error.plot = "upper_errorbar"
    # error.plot = "upper_errorbar", error.plot = "upper_errorbar",
) +
    theme_slick
a.p

# a.p + ggpubr::stat_pvalue_manual(
#     a.res.stats, x = "category", y.position = c(30, 45, 60), label = "p.adj.signif"
# )
