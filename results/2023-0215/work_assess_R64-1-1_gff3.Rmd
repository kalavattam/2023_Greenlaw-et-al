...
TEs <- gff3_SGD %>% dplyr::filter(        
    ...
    dplyr::arrange(seqnames, start, strand)
TEs$Note <- process_list_column(TEs$Note)
TEs$Alias <- process_list_column(TEs$Alias)
TEs$Ontology_term <- process_list_column(TEs$Ontology_term)
TEs$ID <- TEs$Name <- ifelse(
    TEs$type == "five_prime_UTR_intron",
    paste0(TEs$Name, "_5p"),
    ifelse(
        TEs$type == "repeat_region",
        paste(
            TEs$Name,
            paste0(
                TEs$seqnames, ":",
...
#  Group rows after stratifying for 'chr' and 'strand'
TEs_group_chr_strand <- plyr::ddply(
    TEs,
    c("seqnames", "strand"),
    function(x) { 
...
TEs_group_chr_strand$new_group <- is.na(TEs_group_chr_strand$previous_end) | 
    (TEs_group_chr_strand$start >= TEs_group_chr_strand$previous_end)
TEs_group_chr_strand$group <- cumsum(TEs_group_chr_strand$new_group)
TEs_group_chr_strand <- TEs_group_chr_strand %>%
    dplyr::mutate(type_ID = paste0(type, ": ", ID))

#  Aggregate the data
TEs_agg_chr_strand <- plyr::ddply(
    TEs_group_chr_strand,
...
details_Alias <- TEs_agg_chr_strand$Alias %>%
...
details_Name <- TEs_agg_chr_strand$Name %>%
    stringr::str_remove_all("s") %>%
    stringr::str_split("; ") %>%
    purrr::map(sort) %>%
    purrr::map(unique)
...
TEs_agg_chr_strand$Name <- details_Name
TEs_agg_chr_strand$Alias <- details_Alias
TEs_agg_chr_strand$n_types <- ifelse(
    grepl(" ", TEs_agg_chr_strand$Alias, fixed = TRUE),
    stringr::str_count(TEs_agg_chr_strand$Alias, " ") + 1,
    1
)
TEs_agg_chr_strand$n_features <- ifelse(
    grepl(";", TEs_agg_chr_strand$ID, fixed = TRUE),
    stringr::str_count(TEs_agg_chr_strand$ID, ";") + 1,
    1
)

TEs <- TEs_agg_chr_strand
TEs$ID <- gsub("; ", "_", TEs$ID)
TEs$Name <- TEs$Name %>%
    gsub(": ", "-", .) %>%
    gsub("; ", "_", .) %>%
    gsub("TE_gene", "TE", .)
...
rm(TEs_agg_chr_strand, TEs_group_chr_strand)


#  Create a dataframe of TE features antisense to features in features_etc ----
TEs_antisense <- TEs
TEs_antisense$strand <- ifelse(TEs_antisense$strand == "+", "-", "+")
TEs_antisense$source <- "SGD (KA)"
TEs_antisense$type <- paste("antisense", TEs_antisense$type, sep = "_")
TEs_antisense$ID <- paste("AS", TEs_antisense$ID, sep = "_")
TEs_antisense$Name <- paste("AS", TEs_antisense$Name, sep = "_")
...
z_sense <- dplyr::bind_rows(
    features_etc,
    dplyr::select(TEs, -c(n_types, n_features)),
    ARS_etc_plus,
    ARS_etc_minus
) %>%
...
z_sense_antisense <- dplyr::bind_rows(
    ...
    features_etc_antisense,
    dplyr::select(TEs, -c(n_types, n_features)),
    dplyr::select(TEs_antisense, -c(n_types, n_features)),
    ARS_etc_plus,
    ARS_etc_minus
) %>%
    dplyr::arrange(seqnames, start, end, strand) %>%
...
write_gtf(
    format_SGD_tibble(z_antisense),
    paste(p_out, "processed_antisense.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(z_antisense_intergenic),
    paste(p_out, "processed_antisense_intergenic.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(intergenic),
    paste(p_out, "processed_intergenic.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(z_sense),
    paste(p_out, "processed_sense.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(z_sense_antisense),
    paste(p_out, "processed_sense_antisense.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(z_sense_antisense_intergenic),
    paste(p_out, "processed_sense_antisense_intergenic.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(z_sense_intergenic),
    paste(p_out, "processed_sense_intergenic.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(dplyr::select(TEs, -c(n_types, n_features))),
    paste(p_out, "processed_sense_TEs.gtf", sep = "/")
)
write_gtf(
    format_SGD_tibble(dplyr::select(TEs_antisense, -c(n_types, n_features))),
    paste(p_out, "processed_antisense_TEs.gtf", sep = "/")
)
