...
g_Q <- makeGRangesFromDataFrame(
    gtf_Q, keep.extra.columns = TRUE
)
g_G1 <- makeGRangesFromDataFrame(
    gtf_G1, keep.extra.columns = TRUE
)
g_all <- makeGRangesFromDataFrame(
    gtf_all, keep.extra.columns = TRUE
)

o_Q_v_all <- IRanges::findOverlaps(g_Q, g_all)
o_G1_v_all <- IRanges::findOverlaps(g_G1, g_all)

gtf_Q[queryHits(o_Q_v_all), ]
gtf_all[subjectHits(o_Q_v_all), ]
...
wrt_Q_all_group_chr_strand$new_group <- is.na(wrt_Q_all_group_chr_strand$previous_end) | 
    (wrt_Q_all_group_chr_strand$start >= wrt_Q_all_group_chr_strand$previous_end)
wrt_Q_all_group_chr_strand$group <- cumsum(wrt_Q_all_group_chr_strand$new_group)
wrt_Q_all_group_chr_strand <- wrt_Q_all_group_chr_strand %>%
    dplyr::mutate(type_ID = paste0(type_all, ": ", ID_all))

#  Testing out row-wise overlap logic
...
test <- test %>%
    dplyr::relocate(
        c(pct_Q_over_all, pct_all_over_Q, type_ID, group), .after = end_all
    )

test_overlaps <- percent_overlap(test$start, test$end, test$start_all, test$end_all)
test_overlaps %>% table()
