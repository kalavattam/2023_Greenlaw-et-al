...
- "CDS"
- "chromosome"
- "external_transcribed_spacer_region" (they're already in `type` "rRNA")
- "gene_cassette"
- "insertion"
- "internal_transcribed_spacer_region" (they're already in `type` "rRNA")
- "intron"
- "noncoding_exon"
- "nucleotide_match"
- "region"
...
```{r}
#  To start, only want to include the features for which we (may) want to
#+ obtain antisense counts
features_etc <- gff3_SGD %>%
    dplyr::filter(
        # type == "centromere" |  # Handle this separately b/c don't want antisense counts
        type == "five_prime_UTR_intron" |
        type == "gene" |
        ...
        type == "snRNA" |
        type == "snoRNA" |
        # type == "telomere" |  # Handle this separately b/c don't want antisense counts
        type == "transposable_element_gene" |
        type == "tRNA"
    )
features_etc$Note <- process_list_column(features_etc$Note)
features_etc$Parent <- process_list_column(features_etc$Parent)
...
features_etc$ID <- features_etc$Name
...
features_etc_AS <- features_etc
# features_etc_AS %>%
#     dplyr::group_by(strand) %>%
#     dplyr::summarize(n = dplyr::n())

features_etc_AS$strand <- ifelse(features_etc_AS$strand == "+", "-", "+")
features_etc_AS$source <- "SGD (KA)"
features_etc_AS$type <- paste("antisense", features_etc_AS$type, sep = "_")
features_etc_AS$ID <- features_etc_AS$Name <- paste(features_etc_AS$Name, "AS", sep = "_")
...
features_intergenic_etc <- dplyr::bind_rows(
    dplyr::select(features_etc, -c(score, phase)),
    dplyr::select(intergenic, -c(score, phase))
) %>%
    dplyr::arrange(seqnames, start, strand) %>%
    dplyr::filter(seqnames != "2-micron" & seqnames != "Mito")

colnames(features_etc)
colnames(intergenic)
