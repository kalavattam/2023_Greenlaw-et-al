...
Retain `type` "gene_cassette"

Retain `type` "external_transcribed_spacer_region" and "internal_transcribed_spacer_region"
...
- "CDS"
- "chromosome"
- "external_transcribed_spacer_region" (they're already in "rRNA")
- "insertion"
- "internal_transcribed_spacer_region" (they're already in "rRNA")
- "intron"
- "noncoding_exon"
- "nucleotide_match"
- "repeat_region"
...
to_include$Note <- process_list_column(to_include$Note)
to_include$Parent <- process_list_column(to_include$Parent)
to_include$Alias <- process_list_column(to_include$Alias)
to_include$Ontology_term <- process_list_column(to_include$Ontology_term)
to_include$Name <- ifelse(
    to_include$type == "five_prime_UTR_intron",
    paste0(to_include$Name, "_5p"),
    to_include$Name
)
to_include$ID <- ifelse(is.na(to_include$ID), to_include$Name, to_include$ID)

duplicated(to_include$Name) %>% table()
all(to_include$ID == to_include$Name)
