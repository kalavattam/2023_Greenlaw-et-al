...
if(base::isTRUE(load_mRNA_N_Q_r6n)) {
    mRNA_N_Q_r6n <- readRDS(paste(
        p_RDS,
        p_mRNA,
        "mRNA_DGE-analysis_N-Q-rrp6∆_N-Q-WT.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(load_mRNA_SS_Q_r6n)) {
    mRNA_SS_Q_r6n <- readRDS(paste(
        p_RDS,
        p_mRNA,
        "mRNA_DGE-analysis_SS-Q-rrp6∆_SS-Q-WT.rds",
        sep = "/"
    ))
}

load_mRNA_SS_G1_r6n <- FALSE  #ARGUMENT
if(base::isTRUE(load_mRNA_SS_G1_r6n)) {
    mRNA_SS_G1_r6n <- readRDS(paste(
        p_RDS,
        p_mRNA,
        "mRNA_DGE-analysis_SS-G1-rrp6∆_SS-G1-WT.rds",
        sep = "/"
    ))
}
...
load_mRNA_SS_Q_n3d <- FALSE  #ARGUMENT
...
if(base::isTRUE(load_ncRNAcm_N_Q_r6n)) {
    ncRNAcm_N_Q_r6n <- readRDS(paste(
        p_RDS,
        p_ncRNAcm,
        "ncRNA-collapsed_DGE-analysis_N-Q-rrp6∆_N-Q-WT.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(load_ncRNAcm_SS_Q_r6n)) {
    ncRNAcm_SS_Q_r6n <- readRDS(paste(
        p_RDS,
        p_ncRNAcm,
        "ncRNA-collapsed_DGE-analysis_SS-Q-rrp6∆_SS-Q-WT.rds",
        sep = "/"
    ))
}

load_ncRNAcm_SS_G1_r6n <- FALSE  #ARGUMENT
if(base::isTRUE(load_ncRNAcm_SS_G1_r6n)) {
    ncRNAcm_SS_G1_r6n <- readRDS(paste(
        p_RDS,
        p_ncRNAcm,
        "ncRNA-collapsed_DGE-analysis_SS-G1-rrp6∆_SS-G1-WT.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(load_ncRNAcm_N_Q_n3d)) {
    ncRNAcm_N_Q_n3d <- readRDS(paste(
        p_RDS,
        p_ncRNAcm,
        "ncRNA-collapsed_DGE-analysis_N-Q-nab3d_N-Q-parental.rds",
        sep = "/"
    ))
}

load_ncRNAcm_SS_Q_n3d <- FALSE  #ARGUMENT
if(base::isTRUE(load_ncRNAcm_SS_Q_n3d)) {
    ncRNAcm_SS_Q_n3d <- readRDS(paste(
        p_RDS,
        p_ncRNAcm,
        "ncRNA-collapsed_DGE-analysis_SS-Q-nab3d_SS-Q-parental.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(`load_mRNA-AS_N_Q_r6n`)) {
    `mRNA-AS_N_Q_r6n` <- readRDS(paste(
        p_RDS,
        p_mRNA_AS,
        "mRNA-AS_DGE-analysis_N-Q-rrp6∆_N-Q-WT.rds",
        sep = "/"
    ))
...
if(base::isTRUE(`load_mRNA-AS_SS_Q_r6n`)) {
    `mRNA-AS_SS_Q_r6n` <- readRDS(paste(
        p_RDS,
        p_mRNA_AS,
        "mRNA-AS_DGE-analysis_SS-Q-rrp6∆_SS-Q-WT.rds",
        sep = "/"
    ))
    
}

`load_mRNA-AS_SS_G1_r6n` <- FALSE  #ARGUMENT
if(base::isTRUE(`load_mRNA-AS_SS_G1_r6n`)) {
    `mRNA-AS_SS_G1_r6n` <- readRDS(paste(
        p_RDS,
        p_mRNA_AS,
        "mRNA-AS_DGE-analysis_SS-G1-rrp6∆_SS-G1-WT.rds",
        sep = "/"
    ))
...
if(base::isTRUE(`load_mRNA-AS_N_Q_n3d`)) {
    `mRNA-AS_N_Q_n3d` <- readRDS(paste(
        p_RDS,
        p_mRNA_AS,
        "mRNA-AS_DGE-analysis_N-Q-nab3d_N-Q-parental.rds",
        sep = "/"
    ))
}

`load_mRNA-AS_SS_Q_n3d` <- FALSE  #ARGUMENT
if(base::isTRUE(`load_mRNA-AS_SS_Q_n3d`)) {
    `mRNA-AS_SS_Q_n3d` <- readRDS(paste(
        p_RDS,
        p_mRNA_AS,
        "mRNA-AS_DGE-analysis_SS-Q-nab3d_SS-Q-parental.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(`load_R64-etc_N_Q_r6n`)) {
    `R64-etc_N_Q_r6n` <- readRDS(paste(
        p_RDS,
        p_R64_etc,
        "coding-non-pa-ncRNA_DGE-analysis_N-Q-rrp6∆_N-Q-WT.rds",
        sep = "/"
    ))
}

`load_R64-etc_SS_Q_r6n` <- FALSE  #ARGUMENT
if(base::isTRUE(`load_R64-etc_SS_Q_r6n`)) {
    `R64-etc_SS_Q_r6n` <- readRDS(paste(
        p_RDS,
        p_R64_etc,
        "coding-non-pa-ncRNA_DGE-analysis_SS-Q-rrp6∆_SS-Q-WT.rds",
        sep = "/"
    ))
}

`load_R64-etc_SS_G1_r6n` <- FALSE  #ARGUMENT
if(base::isTRUE(`load_R64-etc_SS_G1_r6n`)) {
    `R64-etc_SS_G1_r6n` <- readRDS(paste(
        p_RDS,
        p_R64_etc,
        "coding-non-pa-ncRNA_DGE-analysis_SS-G1-rrp6∆_SS-G1-WT.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(`load_R64-etc_N_Q_n3d`)) {
    `R64-etc_N_Q_n3d` <- readRDS(paste(
        p_RDS,
        p_R64_etc,
        "coding-non-pa-ncRNA_DGE-analysis_N-Q-nab3d_N-Q-parental.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(`load_R64-etc_SS_Q_n3d`)) {
    `R64-etc_SS_Q_n3d` <- readRDS(paste(
        p_RDS,
        p_R64_etc,
        "coding-non-pa-ncRNA_DGE-analysis_SS-Q-nab3d_SS-Q-parental.rds",
        sep = "/"
    ))
}
...
if(base::isTRUE(run)) {
    ...
    #  Analyze ORFs 500-1500 bp in length
    `LFC-ORF-500-1500_N-Q-n3d` <- model_plot_scatter_ORFs(
        df = mRNA_N_Q_n3d$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`[
            mRNA_N_Q_n3d$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`$width >= 500 &
            mRNA_N_Q_n3d$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`$width <= 1500, 
        ],
        color = "#3A538833",
        plot_kb = FALSE,
        xlab = "ORF length (bp)",
    ...
    #  Analyze ORFs 500-1500 bp in length
    `LFC-ORF-500-1500_SS-Q-n3d` <- model_plot_scatter_ORFs(
        df = mRNA_SS_Q_n3d$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`[
            mRNA_SS_Q_n3d$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`$width >= 500 &
            mRNA_SS_Q_n3d$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`$width <= 1500,
        ],
        color = "#481A6C33",
        plot_kb = FALSE,
        xlab = "ORF length (bp)",
    ...
    if(base::isTRUE(check)) {
        ...
        `LFC-ORF-500-1500_N-Q-n3d`$`05_scatter_density`
        `LFC-ORF-500-1500_SS-Q-n3d`$`05_scatter_density`
    }
    ...
    print_scatter(dataframe = `LFC-ORF-500-1500_N-Q-n3d`)
    print_scatter(dataframe = `LFC-ORF-500-1500_SS-Q-n3d`)
}


#  Custom work: For snoRNAs and snRNAs, plot Nab3-AID/parent SS Q LFC vs... ===
#+ Nab3-AID/parent N Q 
run <- FALSE
if(base::isTRUE(run)) {
    #  Load gtf in order to provide biotype context needed for subsetting
    #+ snoRNAs and snRNAs
    ...
    #  Load appropriate dataframes for drawing the scatter plot
    R64_N_Q_n3d <- `R64-etc_N_Q_n3d`$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`
    R64_SS_Q_n3d <- `R64-etc_SS_Q_n3d`$`09_lfc_0.58_fc_1.5`$`04_t_DGE_unshrunken`
    
    #  Add necessary biotype info to dataframes
    R64_N_Q_n3d <- R64_N_Q_n3d %>%
        dplyr::full_join(., t_gff3[, 7:8], by = "features") %>%
        dplyr::select(-type) %>%
        dplyr::rename(chr = seqnames) %>%
        dplyr::relocate(c(features, biotype), .after = strand) %>%
        dplyr::relocate(genome, .before = chr) %>%
        dplyr::select(-c(names, thorough))
    
    R64_SS_Q_n3d <- R64_SS_Q_n3d %>%
        dplyr::full_join(., t_gff3[, 7:8], by = "features") %>%
        dplyr::select(-type) %>%
        dplyr::rename(chr = seqnames) %>%
        dplyr::relocate(c(features, biotype), .after = strand) %>%
    ...
    filt_R64_N_Q_n3d <- R64_N_Q_n3d %>%
        dplyr::filter(biotype %in% c("snoRNA", "snRNA")) %>%
        na.omit()
    
    filt_R64_SS_Q_n3d <- R64_SS_Q_n3d %>%
        dplyr::filter(biotype %in% c("snoRNA", "snRNA")) %>%
        na.omit()
    
    N <- filt_R64_N_Q_n3d
    SS <- filt_R64_SS_Q_n3d
    color <- "#06636833"
    plot_kb <- TRUE
    xlab <- "Nab3-AID/Parent\nlog2(FC) nascent"
    ...
    y_high <- 6
    outpath <- "/Users/kalavatt/Desktop"
    filename <- "scatter.N-on-SS.Q_n3d_sn-snoRNA"
    width <- 7
    height <- 7

    #  Create dataframe of features present in both samples
    #NOTE N should be on x, SS on y
    if(base::isTRUE(plot_kb)) {
        vec_LFC_x <- N[["width"]] / 1000
    } else {
        vec_LFC_x <- N[["width"]]
    }
    vec_LFC_y <- SS[["log2FoldChange"]]
    ...
    #  Draw scatter plot
    scatter <- ggplot(df_xy, aes(x = x, y = y)) +
        geom_hline(  # y = 0
            yintercept = 0,
            linetype = "solid",
            color = "#000000",
    ...
    #  Return results
    results_list <- list()
    results_list[["01_R64_N_Q_n3d"]] <- R64_N_Q_n3d
    results_list[["01_R64_SS_Q_n3d"]] <- R64_SS_Q_n3d
    results_list[["01_filt_R64_N_Q_n3d"]] <- filt_R64_N_Q_n3d
    results_list[["01_filt_R64_SS_Q_n3d"]] <- filt_R64_SS_Q_n3d
    results_list[["02_vec_LFC_x"]] <- vec_LFC_x
    results_list[["02_vec_LFC_y"]] <- vec_LFC_y
    results_list[["02_df_xy"]] <- df_xy
    results_list[["03_a_linear_model"]] <- linear_model
    ...
    readr::write_rds(
        results_list,
        paste(outpath, "N-on-SS.Q_n3d_sn-snoRNA.rds", sep = "/")
    )
}
