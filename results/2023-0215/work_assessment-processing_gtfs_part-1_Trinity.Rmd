---
title: "work_assessment-processing_gtfs_part-1_Trinity.Rmd"
author: "KA"
...
2. Use `htseq-count` to perform feature counting with respect to `type`s "locus", "mRNA", "exon", and "CDS" features from "base" calls to `GffRead`, and with respect to `type` "gene" from "intron-filtering-only" calls to `GffRead`.
3. Read in resulting counts matrices and `gtf` files using custom `R` code; using a mix of `bioconductor` and `tidyverse` programs, perform processing as necessary.
4. Regarding raw, non-normalized alignment-to-putative transcript/transcript fragment (transfrag), retain putative transcripts/transfrags with tallies greater than or equal to a given percentile; in this case, generate results for the 5th to 95th percentiles in iterations of five.
6. Use ~~`rtracklayer`~~ *custom code* to write the converted, collapsed, counts-filtered data objects as `gtf` files. *(Custom code is necessary as some files output by `rtracklayer` have formatting errors that break downstream analyses.)*
7. Send the processed `gtf` files to Alison for inspection, analyses.

For both Q and G1 cell states, Alison asked me to filter the assemblies generated with `Trinity` parameter `mkc` = 1, 4, 8. However, for the sake of completeness, and to understand clearly what "incorrectness" looks like, I will filter `mkc` = 2, 16, 32 as well. Remember that the `Trinity` parameters `mir`, `mg`, and `gf` were held at default levels for the specific assemblies to surveyed by Alison.
</details>
<br />
<br />
...
As a result of calling `gffread` with the `-Z` `-M` `-K` and `-Q` arguments, a new feature is added to the gff3/gtf files: "locus". "locus" is a putative parent feature made from collapsing and joining "children" features (hierarchically, "mRNA" and "exon" features) as described for `-Z` `-M` `-K` and `-Q`. This is an extension of the kind of collapsing performed by `gffcompare -C`. Thus, counts matrices were created with respect to these "locus" features.
...
2. Read in and process counts matrices: "locus", "mRNA", "exon", "CDS", and "gene" (from "intron-only-filtering")
3. Categorize putative transcripts/transfrags by percentile: from percentile 5 to percentile 95 in steps of 5
4. Subset dataframes to retain percentile-filtered IDs: "locus", "mRNA", "exon", "CDS", and "gene" (from "intron-only-filtering")
...
cd /home/kalavatt/tsukiyamalab/kalavatt/2022-2023_RRP6-NAB3/results/2023-0215
...
See [`work_assessment-processing_gtfs_part-0.md`](./work_assessment-processing_gtfs_part-0.md), where *steps 0 through 2 (and an extra step "2.5")* are detailed. Below, we pick up with *step 3*.
<br />
<br />
...
import_g <- function(file) rtracklayer::import(file)
...
rename_columns <- function(list) {
    lapply(
        ...
        function(df) {
            names(df) <-
                gsub("bams_renamed\\/UT_prim_UMI\\/WT_", "", names(df)) %>%
                gsub("_day1_ovn_", "_", .) %>%
                gsub("_day7_ovn_", "_", .) %>% 
                gsub("_aux-F_tc-F_", "_", .) %>%
...
#  Write out gtfs using bespoke approaches/code -------------------------------
for(i in names(out_G_gtf_locus)) {
    for(j in c(paste0(
        ...
        for(k in c(
            ...
            if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "G")) {
                prefix_file <- "/G1_mkc-"
                prefix_message <- "Saving G1"
            } else if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "Q")) {
                prefix_file <- "/Q_mkc-"
                prefix_message <- "Saving Q"
            }
            outfile <- paste0(
                eval(parse(text = k)), prefix_file, gsub("k", "", i), "_",
                gsub("below_", "gte-pctl-", j), ".gtf"
            )
            
            cat(prefix_message, i, j, "in", k, "\n")
            cat(paste0(outfile, "\n"))
            cat("\n\n")
            
            if(base::isTRUE(unlist(stringr::str_split(k, "_"))[3] == "locus")) {
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "G")) {
                    gtf <- out_G_gtf_locus[[i]][[j]]
                }
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "Q")) {
                    gtf <- out_Q_gtf_locus[[i]][[j]]
                }
                gtf <- gtf %>%
                    tibble::as_tibble() %>%
                    dplyr::select(-c(
                        "phase", "width", "geneID", "Name", "coverage",
                        "identity", "matches", "mismatches", "indels",
                        "unknowns", "Target", "CDS_Target", "locus", "Parent",
            ...
            } else if(base::isTRUE(unlist(stringr::str_split(k, "_"))[3] == "mRNA")) {
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "G")) {
                    gtf <- out_G_gtf_mRNA[[i]][[j]]
                }
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "Q")) {
                    gtf <- out_Q_gtf_mRNA[[i]][[j]]
                }
                gtf <- gtf %>%
                    tibble::as_tibble() %>%
                    dplyr::select(-c(
                    ...
                    dplyr::select(-c(
                        id, Name, gene_id, locus, coverage, identity, matches,
                        mismatches, indels, unknowns, Target, CDS_Target,
                        partialness, InFrameStop
                    ))
            } else if(base::isTRUE(unlist(stringr::str_split(k, "_"))[3] == "exon")) {
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "G")) {
                    gtf <- out_G_gtf_exon[[i]][[j]]
                }
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "Q")) {
                    gtf <- out_Q_gtf_exon[[i]][[j]]
                }
                gtf <- gtf %>%
                    tibble::as_tibble() %>%
                    dplyr::select(-c(
                        "width", "phase", "ID", "geneIDs", "transcripts",
                        "geneID", "Name", "coverage", "identity", "matches",
                        "mismatches", "indels", "unknowns", "CDS_Target",
                        "partialness", "locus", "InFrameStop"
                    )) %>%
                    dplyr::arrange(seqnames, start) %>%
            ...
            } else if(base::isTRUE(unlist(stringr::str_split(k, "_"))[3] == "CDS")) {
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "G")) {
                    gtf <- out_G_gtf_CDS[[i]][[j]]
                }
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "Q")) {
                    gtf <- out_Q_gtf_CDS[[i]][[j]]
                }
                gtf <- gtf %>%
                    tibble::as_tibble() %>%
                    dplyr::select(-c(
                        "width", "ID", "geneIDs", "transcripts", "geneID",
                        "Name", "coverage", "identity", "matches",
                        "mismatches", "indels", "unknowns", "CDS_Target",
                        "partialness", "locus", "InFrameStop"
                    )) %>%
                    dplyr::arrange(seqnames, start) %>%
                    ...
                    dplyr::mutate(
                        source = paste0(
                            source,
                            "; trinity; gffread; Greenlaw et al."
                        ),
                        score = ".",
                        frame = ".",
                        id = process_list_column(id)
            ...
            } else if(base::isTRUE(unlist(stringr::str_split(k, "_"))[3] == "introns")) {
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "G")) {
                    gtf <- out_G_gtf_introns_filtered[[i]][[j]]
                }
                if(base::isTRUE(unlist(stringr::str_split(k, "_"))[2] == "Q")) {
                    gtf <- out_Q_gtf_introns_filtered[[i]][[j]]
                }
                gtf <- gtf %>%
                    tibble::as_tibble() %>%
            ...
            write_gtf(gtf, outfile)
        }
    }
}

# rm(i, j, k, outfile, gtf)
```
<br />

```{r, eval=FALSE}
#  Write out gtfs using rtracklayer::export() ---------------------------------
#DONOTRUN #PREVIOUS
for(i in names(out_G_gtf_locus)) {
    for(j in c(paste0(
        "below_",
...
rm(i, j, k, outfile, out_G_conv, out_Q_conv)
```
</details>
<br />
