...
#  rough-draft_pre-filter-matrices_etc.R
#  KA
...
call_DESeq2_results_plot_volcano <- function(
    ...
    # :param dds: ...
    # :return ...: ...
    if(base::isFALSE(is.logical(shrink))) {
        stop("Argument \"shrink\" must be TRUE or FALSE <lgl>")
    }
    ...
    if(isFALSE(shrink)) {
        ...
        subtitle <- paste(
            "points: S. cerevisiae features",
            "| size factors (RLE): K. lactis features",
            "\nsamples:", sample_info, "| apeglm estimation",
            "| model: ~", paste(as.character(model_info)[-1], collapse = " + "),
            "\nleft: up in", name_ctrl,
            "| right: up in", name_exp
        )
        
        p_shrunken <- plot_volcano(
    ...
    if(base::isTRUE(shrink)) {
        results_list[["04_DGE_shrunken_DF"]] <- DGE_shrunken_DF
        results_list[["05_DGE_shrunken_GR"]] <- DGE_shrunken_GR
    }
    results_list[["06_t_DGE_unshrunken"]] <- t_DGE_unshrunken
    if(base::isTRUE(shrink)) {
        results_list[["07_t_DGE_shrunken"]] <- t_DGE_shrunken
    }
    if(base::isTRUE(selection)) {
        results_list[["08_selection_down_unshrunken"]] <- selection_down_unshrunken
        results_list[["09_selection_up_unshrunken"]] <- selection_up_unshrunken
        results_list[["10_selection_unshrunken"]] <- selection_unshrunken
        
        if(base::isTRUE(shrink)) {
            results_list[["11_selection_down_shrunken"]] <- selection_down_shrunken
            results_list[["12_selection_up_shrunken"]] <- selection_up_shrunken
            results_list[["12_selection_shrunken"]] <- selection_shrunken
        }
    }
    results_list[["14_p_unshrunken"]] <- p_unshrunken
    if(base::isTRUE(shrink)) {
        results_list[["15_p_shrunken"]] <- p_shrunken
    }
...
t_xslx <- paste(p_base, p_exp, p_xlsx, f_xlsx, sep = "/") %>%
    readxl::read_xlsx()  #FIXME #LATER In this file, replicate information for 5781, 5782, 7079, 7078, 6125, 6126, 7718, and 7716 is incorrect
