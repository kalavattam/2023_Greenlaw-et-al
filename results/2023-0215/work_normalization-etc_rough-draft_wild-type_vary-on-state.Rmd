---
title: "work_normalization-etc_rough-draft_wild-type_vary-on-state"
author: "KA"
...
```{r Set working directory, results='hide', echo=FALSE,  message=FALSE}
...
```{r Load necessary libraries, results='hide', echo=FALSE,  message=FALSE, warning=FALSE}
library(DESeq2)
...
```{r Initialize necessary functions, results='hide', echo=FALSE,  message=FALSE}
...
etc. work
```{r load spreadsheet, results='hide', echo=FALSE,  message=FALSE}
...
```{r Process featureCounts table, results='hide', echo=FALSE,  message=FALSE}
...
```{r Record positional information, results='hide', echo=FALSE,  message=FALSE}
...
```{r Make a master model matrix, results='hide', echo=FALSE,  message=FALSE}
...
```{r Make the counts matrix, results='hide', echo=FALSE,  message=FALSE}
datasets <- c(
...
```{r make model matrix, results='hide', echo=FALSE,  message=FALSE}
...
```{r make dds, results='hide', echo=FALSE,  message=FALSE}
...
#TODO #LOWPRIORITY 1/3 Try out complicated models for Q vs G1 w/r/t N vs. SS 
#TODO #LOWPRIORITY 2/3 later; e.g., may want to test
#TODO #LOWPRIORITY 3/3 `design = ~ state + transcription + state:transcription`
```
<br />
...
```{r prefilter dds, results='hide', echo=FALSE,  message=FALSE}
# threshold <- 0
...
```{r I. Do rlog normalization, results='hide', echo=FALSE,  message=FALSE}
...
```{r I. Call DESeq2 using default parameters, results='hide', echo=TRUE, message=FALSE}
dds_SC <- DESeq2::DESeq(dds_SC)

#  Check model information
DESeq2::resultsNames(dds_SC)[2]
#  Model varies on state, Q is the numerator, G1 is the denominator
```
<br />
...
#### Make an MA plot
```{r I. Make an MA plot: A, message=FALSE}
#REMEMBER 1/2 No `controlGenes` were used; the calculation of size factors was
#REMEMBER 2/2 performed with S. cerevisiae features (all)
# DESeq2::plotMA(
...
#     frame = FALSE,
# )
#  Plot of only S. cerevisiae points (K. lactis points have already been
#+ excluded)
#+     - top: up in Q
#+     - bottom: up in G1
...
#     geom_point(alpha = 0.25, size = 0.5) +
#     geom_point(
#         data = dplyr::filter(tbl, stably_expressed == TRUE),
#         pch = 21,
#         size = 1.5,
#         color = "#00000090"
#     ) +
...
#     scale_colour_discrete(name = "q ≤ 0.05") +
#     ggtitle(
#         "MA plot: \"Stably expressed\" features in circles",
#         subtitle = "dots: S. cerevisiae features | top: Q | bottom: G1"
#     ) +
#     theme_slick
...
subtitle <- paste(
    ...
    "| left: up in G1",
    "| right: up in Q",
    "| labels: top 5 G1 and top 5 Q features"
)
p_SC <- plot_volcano(
    table = t_DGE_SC,
    label = all,
...
```{r II. Perform size-factor estimation, results='hide', echo=TRUE, message=FALSE}
...
#### Do rlog normalization
```{r II. Do rlog normalization, results='hide', echo=FALSE,  message=FALSE}
...
### Run `DESeq2`
#### Call `DESeq2` using default parameters
```{r II. Call DESeq2 using default parameters, results='hide', echo=TRUE, message=FALSE}
...
#### Make an MA plot
```{r II. Make an MA plot: A, message=FALSE}
#REMEMBER 1/2 No `controlGenes` were used; the calculation of size factors was
#REMEMBER 2/2 performed with all K. lactis features
# DESeq2::plotMA(
...
#     frame = FALSE,
# )
#  Plot of only K. lactis points (S. cerevisiae points have already been
#+ excluded)
#+     - top: up in Q
#+     - bottom: up in G1
...
#     ggtitle(
#         "MA plot: \"Stably expressed\" features in circles",
#         subtitle = "dots: K. lactis features | top: Q | bottom: G1"
#     ) +
#     theme_slick
...
```{r III. Perform size-factor estimation, results='hide', echo=TRUE, message=FALSE}
...
#NOTE 1/2 Note the much smaller (G1) and much larger (Q) coefficients needed to
...
```{r III. Do rlog normalization, results='hide', echo=FALSE,  message=FALSE}
# rlog_F_SC.ctrl_KL <- DESeq2::rlog(
...
#     dplyr::bind_cols(
#         .,
#         tibble::as_tibble(dds_SC.ctrl_KL@rowRanges)[
#             dds_SC.ctrl_KL@rowRanges$genome != "K_lactis", 
#         ]
#     )
# 
# rlog_T_SC.ctrl_KL <- DESeq2::rlog(
#     dds_SC.ctrl_KL[dds_SC.ctrl_KL@rowRanges$genome != "K_lactis", ],
#     blind = TRUE
# ) %>%
#     SummarizedExperiment::assay() %>%
...
```{r III. Call DESeq2 using default parameters, results='hide', echo=TRUE, message=FALSE}
...
```{r III. Make an MA plot: A, message=FALSE}
#REMEMBER 1/2 All K. lactis features were used as `controlGenes`; calculation
#REMEMBER 2/2 of size factors with K. lactis features only
# DESeq2::plotMA(
#     DGE_unshrunken_DF_SC.ctrl_KL,
#     colNonSig = "#D3D3D390",
#     colSig = "#A020F090",
#     colLine = "#82858733",
#     ylim = c(-18, 18),
#     frame = FALSE,
# )
#  Plot of only S. cerevisiae points (K. lactis points have been excluded)
#+     - top: up in Q
#+     - bottom: up in G1
```
<br />
...
## IV. Run analyses of *S.C.*: use stably expressed *K.L.* as `controlGenes`
### Perform size-factor estimation
To calculate the size factors using stable *K.lactis* features as
`controlGenes`, we use the stably expressed features determined in an above
code chunk and saved in variable `stably_expressed_KL`.

```{r IV. Perform size-factor estimation, results='hide', echo=TRUE, message=FALSE}
#  Recalculate size factors using "stably expressed" K. lactis features as
#+ `controlGenes`
dds_SC.ctrl_KL_stab <- BiocGenerics::estimateSizeFactors(
    dds,
    controlGenes = (dds@rowRanges$feature %in% stably_expressed_KL)
)
dds_SC.ctrl_KL_stab@colData
#  Using "stably expressed" K. lactis features as `controlGenes`
# G1 rep1 0.170220
# G1 rep2 0.331943
#  Q rep1 5.046135
#  Q rep2 3.522188
...
```{r IV. Do rlog normalization, results='hide', echo=FALSE,  message=FALSE}
# rlog_F_SC.ctrl_KL_stab <- DESeq2::rlog(
#     dds_SC.ctrl_KL_stab[
#         dds_SC.ctrl_KL_stab@rowRanges$genome != "K_lactis", 
#     ],
#     blind = FALSE
# ) %>%
#     SummarizedExperiment::assay() %>%
...
#     dplyr::bind_cols(
#         .,
#         tibble::as_tibble(dds_SC.ctrl_KL_stab@rowRanges)[
#             dds_SC.ctrl_KL_stab@rowRanges$genome != "K_lactis",
#         ]
#     )
# 
# rlog_T_SC.ctrl_KL_stab <- DESeq2::rlog(
#     dds_SC.ctrl_KL_stab[
#         dds_SC.ctrl_KL_stab@rowRanges$genome != "K_lactis", 
#     ],
#     blind = TRUE
...
# 
# #  How does rlog_T look?
# rlog_T_SC.ctrl_KL_stab
```
<br />

### Run `DESeq2`
#### Call `DESeq2` using default parameters
Since we've already calculated the size factors, we exclude *K. lactis*
features from our work from here on out.
```{r IV. Call DESeq2 using default parameters, results='hide', echo=TRUE, message=FALSE}
dds_SC.ctrl_KL_stab <- DESeq2::DESeq(
    dds_SC.ctrl_KL_stab[dds_SC.ctrl_KL_stab@rowRanges$genome != "K_lactis", ]
)

#  Check model information
DESeq2::resultsNames(dds_SC.ctrl_KL_stab)[2]
#  Model varies on state, Q is the numerator, G1 is the denominator
```
<br />

#### Call `DESeq2::results()`
```{r IV. Call DESeq2::results(), message=FALSE}
#  Set up necessary parameters for generation of DESeq2 results table
independent_filtering <- TRUE
threshold_p <- 0.05
threshold_lfc <- 0
...
#  Output a DESeq2 DataFrame object
DGE_unshrunken_DF_SC.ctrl_KL_stab <- DESeq2::results(
    dds_SC.ctrl_KL_stab,
    name = DESeq2::resultsNames(dds_SC.ctrl_KL_stab)[2],
    independentFiltering = independent_filtering,
    alpha = threshold_p,
    lfcThreshold = threshold_lfc,
    format = "DataFrame"
...
#  Output a GRanges object, which we can easily add to and convert to other
#+ formats (such as a tibble)
DGE_unshrunken_GR_SC.ctrl_KL_stab <- DESeq2::results(
    dds_SC.ctrl_KL_stab,
    name = DESeq2::resultsNames(dds_SC.ctrl_KL_stab)[2],
    independentFiltering = independent_filtering,
    alpha = threshold_p,
...
DGE_unshrunken_GR_SC.ctrl_KL_stab$feature <-
    MatrixGenerics::rowRanges(dds_SC.ctrl_KL_stab)$feature
DGE_unshrunken_GR_SC.ctrl_KL_stab$type <-
    MatrixGenerics::rowRanges(dds_SC.ctrl_KL_stab)$type
DGE_unshrunken_GR_SC.ctrl_KL_stab$genome <-
    MatrixGenerics::rowRanges(dds_SC.ctrl_KL_stab)$genome

t_DGE_SC.ctrl_KL_stab <- DGE_unshrunken_GR_SC.ctrl_KL_stab %>%
    dplyr::as_tibble()
...
#### Make an MA plot
```{r IV. Make an MA plot: A, message=FALSE}
#REMEMBER Stably expressed K. lactis features were used as `controlGenes`
# DESeq2::plotMA(
#     DGE_unshrunken_DF_SC.ctrl_KL_stab,
#     colNonSig = "#D3D3D390",
#     colSig = "#DC267F90",
#     colLine = "#82858733",
#     ylim = c(-18, 18),
...
#### Make an MA plot that colors features by independent filtering
```{r IV. Make an MA plot: B, message=FALSE}
#  Set up temporary variable 'tbl', which will be passed to ggplot
tbl <- t_DGE_SC.ctrl_KL_stab
tbl <- tbl[with(tbl, order(log2FoldChange)), ]
tbl$threshold <- as.factor(tbl$padj <= 0.05)
tbl$log10baseMean <- ifelse(
    is.infinite(log10(tbl$baseMean)), NA, log10(tbl$baseMean)
...
title <- paste0(
    "MA plot | S. cerevisiae features |\n",
    "size factors estimated with \"stably expressed\" K. lactis features"
)
subtitle <- paste(
    "points: S. cerevisiae features",
    "| top: up in Q",
...
stably_expressed_SC.ctrl_KL_stab <- tbl$stably_expressed[
    !is.na(tbl$stably_expressed)
]
...
#### Make a volcano plot
```{r IV. Make a volcano plot, message=FALSE}
all <- t_DGE_SC.ctrl_KL_stab$feature
selection_down <- t_DGE_SC.ctrl_KL_stab %>%
    dplyr::filter(log2FoldChange < 0) %>%
    dplyr::arrange(padj) %>%
    dplyr::slice(1:5)
selection_up <- t_DGE_SC.ctrl_KL_stab %>%
    dplyr::filter(log2FoldChange > 0) %>%
    dplyr::arrange(padj) %>%
    dplyr::slice(1:5)
selection <- c(selection_down[["feature"]], selection_up[["feature"]]) %>%
...
p_SC.ctrl_KL_stab <- plot_volcano(
    table = t_DGE_SC.ctrl_KL_stab,
    label = all,
    selection = selection,
    label_size = 2.5,
    p_cutoff = 0.05,
...
## V. Run analyses with both *S. cerevisae* and *K. lactis* features only
### Perform size-factor estimation
Here, we use all features, both *S. cerevisae* and *K. lactis*, for size-factor
estimation.

```{r V. Perform size-factor estimation, message=FALSE}
dds_all <- BiocGenerics::estimateSizeFactors(dds)
dds_all@colData
#  Not using `controlGenes`; calculating size factors with all S. cerevisiae
#+ and K. lactis features
# G1 rep1 0.622333
# G1 rep2 0.638943
#  Q rep1 1.522589
#  Q rep2 1.687240
...
#### Do rlog normalization
```{r V. Do rlog normalization, results='hide', echo=FALSE,  message=FALSE}
# rlog_F_all <- DESeq2::rlog(
#     dds_all,
#     blind = FALSE
# ) %>%
#     SummarizedExperiment::assay() %>%
...
#     dplyr::bind_cols(
#         .,
#         tibble::as_tibble(dds_all@rowRanges)
#     )
# 
# rlog_T_all <- DESeq2::rlog(
#     dds_all,
#     blind = TRUE
# ) %>%
#     SummarizedExperiment::assay() %>%
...
### Run `DESeq2`
#### Call `DESeq2` using default parameters
```{r V. Call DESeq2 using default parameters, results='hide', echo=TRUE, message=FALSE}
dds_all <- DESeq2::DESeq(dds_all)

#  Check model information
DESeq2::resultsNames(dds_all)[2]
#  Model varies on state, Q is the numerator, G1 is the denominator
```
<br />
...
```{r V. Call DESeq2::results(), message=FALSE}
#  Set up necessary parameters for generation of DESeq2 results table
independent_filtering <- TRUE
threshold_p <- 0.05
threshold_lfc <- 0
...
#  Output a DESeq2 DataFrame object
DGE_unshrunken_DF_all <- DESeq2::results(
    dds_all,
    name = DESeq2::resultsNames(dds_all)[2],
    independentFiltering = independent_filtering,
    alpha = threshold_p,
    lfcThreshold = threshold_lfc,
    format = "DataFrame"
...
#  Output a GRanges object, which we can easily add to and convert to other
#+ formats (such as a tibble)
DGE_unshrunken_GR_all <- DESeq2::results(
    dds_all,
    name = DESeq2::resultsNames(dds_all)[2],
    independentFiltering = independent_filtering,
    alpha = threshold_p,
...
DGE_unshrunken_GR_all$feature <- MatrixGenerics::rowRanges(dds_all)$feature
DGE_unshrunken_GR_all$type <- MatrixGenerics::rowRanges(dds_all)$type
DGE_unshrunken_GR_all$genome <- MatrixGenerics::rowRanges(dds_all)$genome

t_DGE_all <- DGE_unshrunken_GR_all %>%
    dplyr::as_tibble() %>%
    dplyr::rename(chr = seqnames)
...
#### Make an MA plot
```{r V. Make an MA plot: A, message=FALSE}
#REMEMBER All features were used for size-factor estimation
# DESeq2::plotMA(
#     DGE_unshrunken_DF_all,
#     colNonSig = "#D3D3D390",
#     colSig = "#0D98BA90",
#     colLine = "#82858733",
#     ylim = c(-14, 14),
#     frame = FALSE,
# )
#  Plot of all points, S. cerevisiae and K. lactis
#+     - top: up in Q
#+     - bottom: up in G1

#  Seeing what looks like a bimodal distribution, one distribution above zero
#+ and another larger one below zero
```
<br />

#### Make an MA plot that colors features by independent filtering
```{r V. Make an MA plot: B, message=FALSE}
#  Set up temporary variable 'tbl', which will be passed to ggplot
tbl <- t_DGE_all
tbl <- tbl[with(tbl, order(log2FoldChange)), ]
tbl$threshold <- as.factor(tbl$padj <= 0.05)
...
title <- paste0(
    "MA plot | S. cerevisiae and K. lactis features |\n",
    "size factors estimated with all features"
)
subtitle <- paste(
    "points: S. cerevisiae and K. lactis features",
    "| top: up in Q",
    "| bottom: up in G1"
)
ggplot(tbl, aes(x = log10baseMean, y = log2FoldChange, colour = threshold)) +
...
stably_expressed_all <- tbl$feature[tbl$stably_expressed == TRUE]
#NOTE Probably need to be more stringent here
...
#### Make a volcano plot
```{r V. Make a volcano plot, message=FALSE}
#  Identify and isolate the top 5 upregulated features and the top 5 down-
#+ regulated features
all <- t_DGE_all$feature
selection_down <- t_DGE_all %>%
    dplyr::filter(log2FoldChange < 0) %>%
    dplyr::arrange(padj) %>%
    dplyr::slice(1:5)
selection_up <- t_DGE_all %>%
    dplyr::filter(log2FoldChange > 0) %>%
    dplyr::arrange(padj) %>%
    dplyr::slice(1:5)
selection <- c(selection_down[["feature"]], selection_up[["feature"]]) %>%
...
title <- paste0(
    "volcano plot | S. cerevisiae and K. lactis features |\n",
    "size factors estimated with all features"
)
subtitle <- paste(
    "points: S. cerevisiae and K. lactis features",
