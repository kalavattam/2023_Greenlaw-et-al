...
#NOTE
#  We can nearly reproduce the PCA results in
#+ notebook/KA.2023-0324.presentation_QC with the following model supplied to
#+ DESeq2::DESeqDataSetFromMatrix(): ~ technical + genotype
#+
#+ PC3 is significant and accounts for ~9.5% of variation; it's tightly
#+ correlated with genotype
#+ 
#+ We seem to sidestep problems when we include technical in the model when
#+ making the dds, rather than correcting after the fact as we do in the chunk
...
#  DSm2 vs. DSp2 --------------------------------------------------------------
vs_DSm2_DSp2_raw <- counts_raw[
    , stringr::str_detect(colnames(counts_raw), "DSm2_|DSp2_")
]
vs_DSm2_DSp2_rlog <- counts_rlog[
    , stringr::str_detect(colnames(counts_raw), "DSm2_|DSp2_")
]

pca_DSm2_DSp2_rlog <- run_PCA_pipeline(
    counts = vs_DSm2_DSp2_rlog,
    metadata = t_meta[rownames(t_meta) %in% colnames(vs_DSm2_DSp2_rlog), ],
    gene_id = gene_id,
    transformed = TRUE,
    transcription = FALSE,
    meta_color = "genotype",
...
pca_DSm2_DSp2_rlog[["02_horn"]]$n
pca_DSm2_DSp2_rlog[["03_elbow"]]
pca_DSm2_DSp2_rlog[["04_p_scree"]]
pca_DSm2_DSp2_rlog[["10_p_images"]][["PCAtools.PC1.v.PC2"]]
pca_DSm2_DSp2_rlog[["10_p_images"]][["KA.PC1.v.PC2"]]
pca_DSm2_DSp2_rlog[["12_p_cor"]]

#  Write out loadings
#TODO Function-ize this 
important_names <- t_tc_SC[, c(8, 10)]
important_names$IDs <- ifelse(
    is.na(important_names$names),
    important_names$features,
    important_names$names
)

PC_DSm2_DSp2_rlog_PC1_pos <- get_top_loadings(
    x = pca_DSm2_DSp2_rlog$`05_loadings`,
    y = "PC1",
    z = "pos",
    a = FALSE
) %>%
    tibble::rownames_to_column(var = "IDs")
PC_DSm2_DSp2_rlog_PC1_pos <- dplyr::full_join(
    PC_DSm2_DSp2_rlog_PC1_pos, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC1") %>%
    dplyr::relocate("PC1", .before = "IDs")

PC_DSm2_DSp2_rlog_PC1_neg <- get_top_loadings(
    x = pca_DSm2_DSp2_rlog$`05_loadings`,
    y = "PC1",
    z = "neg",
    a = FALSE
    ...
    tibble::rownames_to_column(var = "IDs")
PC_DSm2_DSp2_rlog_PC1_neg <- dplyr::full_join(
    PC_DSm2_DSp2_rlog_PC1_neg, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC1") %>%
    dplyr::relocate("PC1", .before = "IDs")

PC_DSm2_DSp2_rlog_PC2_pos <- get_top_loadings(
    x = pca_DSm2_DSp2_rlog$`05_loadings`,
    y = "PC2",
    z = "pos",
    a = FALSE
    ...
    tibble::rownames_to_column(var = "IDs")
PC_DSm2_DSp2_rlog_PC2_pos <- dplyr::full_join(
    PC_DSm2_DSp2_rlog_PC2_pos, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC2") %>%
    dplyr::relocate("PC2", .before = "IDs")

PC_DSm2_DSp2_rlog_PC2_neg <- get_top_loadings(
    x = pca_DSm2_DSp2_rlog$`05_loadings`,
    y = "PC2",
    z = "neg",
    a = FALSE
    ...
    tibble::rownames_to_column(var = "IDs")
PC_DSm2_DSp2_rlog_PC2_neg <- dplyr::full_join(
    PC_DSm2_DSp2_rlog_PC2_neg, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC2") %>%
    dplyr::relocate("PC2", .before = "IDs")
...
#  DSp2 vs. DSp24 -------------------------------------------------------------
vs_DSp2_DSp24_raw <- counts_raw[
    , stringr::str_detect(colnames(counts_raw), "DSp2_|DSp24_")
]
vs_DSp2_DSp24_rlog <- counts_rlog[
    , stringr::str_detect(colnames(counts_raw), "DSp2_|DSp24_")
]

pca_DSp2_DSp24_rlog <- run_PCA_pipeline(
    counts = vs_DSp2_DSp24_rlog,
    metadata = t_meta[rownames(t_meta) %in% colnames(vs_DSp2_DSp24_rlog), ],
    gene_id = gene_id,
    transformed = TRUE,
    transcription = FALSE,
    meta_color = "genotype",
...
pca_DSp2_DSp24_rlog[["02_horn"]]$n
pca_DSp2_DSp24_rlog[["03_elbow"]]
pca_DSp2_DSp24_rlog[["04_p_scree"]]
pca_DSp2_DSp24_rlog[["10_p_images"]][["PCAtools.PC1.v.PC2"]]
pca_DSp2_DSp24_rlog[["10_p_images"]][["KA.PC1.v.PC2"]]
pca_DSp2_DSp24_rlog[["12_p_cor"]]
...
PC_DSp2_DSp24_rlog_PC1_pos <- get_top_loadings(
    x = pca_DSp2_DSp24_rlog$`05_loadings`,
    y = "PC1",
    z = "pos",
    a = FALSE
) %>%
    tibble::rownames_to_column(var = "IDs")
PC_DSp2_DSp24_rlog_PC1_pos <- dplyr::full_join(
    PC_DSp2_DSp24_rlog_PC1_pos, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC1") %>%
    dplyr::relocate("PC1", .before = "IDs")

PC_DSp2_DSp24_rlog_PC1_neg <- get_top_loadings(
    x = pca_DSp2_DSp24_rlog$`05_loadings`,
    y = "PC1",
    z = "neg",
    a = FALSE
    ...
    tibble::rownames_to_column(var = "IDs")
PC_DSp2_DSp24_rlog_PC1_neg <- dplyr::full_join(
    PC_DSp2_DSp24_rlog_PC1_neg, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC1") %>%
    dplyr::relocate("PC1", .before = "IDs")

PC_DSp2_DSp24_rlog_PC2_pos <- get_top_loadings(
    x = pca_DSp2_DSp24_rlog$`05_loadings`,
    y = "PC2",
    z = "pos",
    a = FALSE
    ...
    tibble::rownames_to_column(var = "IDs")
PC_DSp2_DSp24_rlog_PC2_pos <- dplyr::full_join(
    PC_DSp2_DSp24_rlog_PC2_pos, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC2") %>%
    dplyr::relocate("PC2", .before = "IDs")

PC_DSp2_DSp24_rlog_PC2_neg <- get_top_loadings(
    x = pca_DSp2_DSp24_rlog$`05_loadings`,
    y = "PC2",
    z = "neg",
    a = FALSE
    ...
    tibble::rownames_to_column(var = "IDs")
PC_DSp2_DSp24_rlog_PC2_neg <- dplyr::full_join(
    PC_DSp2_DSp24_rlog_PC2_neg, important_names, by = "IDs"
) %>%
    tidyr::drop_na("PC2") %>%
    dplyr::relocate("PC2", .before = "IDs")
