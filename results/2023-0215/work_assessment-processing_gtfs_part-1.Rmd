---
title: "work_assessment-processing_gtfs.Rmd"
author: "KA"
...
See [`#work_assessment-processing_gtfs_part-0.md`](./work_assessment-processing_gtfs_part-0.md), where *steps 0 through 4* are detailed. Below, we pick up with *step 5*.
<br />
<br />
...
files_G_introns_filtered <- list.files(
    path_gtf_G, pattern = "*.gffread-intron-filtering-only.gff3", full.names = TRUE
)
...
files_Q_introns_filtered <- list.files(
    path_gtf_Q, pattern = "*.gffread-intron-filtering-only.gff3", full.names = TRUE
)
...
files_G_exon <- list.files(  # Approach #2
    path_mat_G, pattern = "*-exon.tsv", full.names = TRUE
)
l_G_mat_exon <- read_in_mat(files_G_exon) %>% suppressMessages()

files_G_CDS <- list.files(  # Approach #2
    path_mat_G, pattern = "*-CDS.tsv", full.names = TRUE
)
l_G_mat_CDS <- read_in_mat(files_G_CDS) %>% suppressMessages()

files_G_introns_filtered <- list.files(
    path_mat_G, pattern = "*.gffread-intron-filtering-only.hc-strd-eq-gene.tsv", full.names = TRUE
)
l_G_mat_introns_filtered <- read_in_mat(files_G_introns_filtered) %>% suppressMessages()
...
files_Q_exon <- list.files(  # Approach #2
    path_mat_Q, pattern = "*-exon.tsv", full.names = TRUE
)
l_Q_mat_exon <- read_in_mat(files_Q_exon) %>% suppressMessages()

files_Q_CDS <- list.files(  # Approach #2
    path_mat_Q, pattern = "*-CDS.tsv", full.names = TRUE
)
l_Q_mat_CDS <- read_in_mat(files_Q_CDS) %>% suppressMessages()

files_Q_introns_filtered <- list.files(
    path_mat_Q, pattern = "*.gffread-intron-filtering-only.hc-strd-eq-gene.tsv", full.names = TRUE
)
l_Q_mat_introns_filtered <- read_in_mat(files_Q_introns_filtered) %>% suppressMessages()
...
l_G_mat_exon <- rename_columns(l_G_mat_exon)  # Approach #2
l_G_mat_CDS <- rename_columns(l_G_mat_CDS)  # Approach #2
...
l_Q_mat_exon <- rename_columns(l_Q_mat_exon)  # Approach #2
l_Q_mat_CDS <- rename_columns(l_Q_mat_CDS)  # Approach #2
...
f_G_mat_exon <- sapply(  # Approach #2
    l_G_mat_exon,
    filter_transfrags,
    simplify = FALSE,
    USE.NAMES = TRUE,
...
f_G_mat_introns_filtered <- sapply(  # Approach #2
...
f_Q_mat_exon <- sapply(  # Approach #2
    l_Q_mat_exon,
    filter_transfrags,
    simplify = FALSE,
    USE.NAMES = TRUE,
...
f_Q_mat_introns_filtered <- sapply(  # Approach #2
...
out_G_gtf_exon <- subset_dataframes(l_G_gtf, f_G_mat_exon)  # Approach #2
out_G_gtf_CDS <- subset_dataframes(l_G_gtf, f_G_mat_CDS)  # Approach #2
out_G_gtf_introns_filtered <- subset_dataframes(l_G_gtf_introns_filtered, f_G_mat_introns_filtered)  # Approach #2
...
out_Q_gtf_exon <- subset_dataframes(l_Q_gtf, f_Q_mat_exon)  # Approach #2
out_Q_gtf_CDS <- subset_dataframes(l_Q_gtf, f_Q_mat_CDS)  # Approach #2
out_Q_gtf_introns_filtered <- subset_dataframes(l_Q_gtf_introns_filtered, f_Q_mat_introns_filtered)  # Approach #2
```
</details>
<br />
...
path_G_mRNA <- "outfiles_gtf-gff3/Trinity-GG/G_N/filtered/mRNA"  # Approach #2
if(isFALSE(dir.exists(path_G_mRNA))) dir.create(path_G_mRNA, recursive = TRUE)

path_G_exon <- "outfiles_gtf-gff3/Trinity-GG/G_N/filtered/exon"  # Approach #2
if(isFALSE(dir.exists(path_G_exon))) dir.create(path_G_exon, recursive = TRUE)

path_G_CDS <- "outfiles_gtf-gff3/Trinity-GG/G_N/filtered/CDS"  # Approach #2
if(isFALSE(dir.exists(path_G_CDS))) dir.create(path_G_CDS, recursive = TRUE)

path_G_introns_filtered <- "outfiles_gtf-gff3/Trinity-GG/G_N/filtered/introns_filtered"  # Approach #2
...
path_Q_mRNA <- "outfiles_gtf-gff3/Trinity-GG/Q_N/filtered/mRNA"  # Approach #2
if(isFALSE(dir.exists(path_Q_mRNA))) dir.create(path_Q_mRNA, recursive = TRUE)

path_Q_exon <- "outfiles_gtf-gff3/Trinity-GG/Q_N/filtered/exon"  # Approach #2
if(isFALSE(dir.exists(path_Q_exon))) dir.create(path_Q_exon, recursive = TRUE)

path_Q_CDS <- "outfiles_gtf-gff3/Trinity-GG/Q_N/filtered/CDS"  # Approach #2
if(isFALSE(dir.exists(path_Q_CDS))) dir.create(path_Q_CDS, recursive = TRUE)

path_Q_introns_filtered <- "outfiles_gtf-gff3/Trinity-GG/Q_N/filtered/introns_filtered"  # Approach #2
...
for(i in names(out_G_gtf_locus)) {
    for(j in c(paste0(
        ...
        for(k in c(
            path_G_locus, path_G_mRNA, path_G_exon, path_G_CDS, path_G_introns_filtered,
            path_Q_locus, path_Q_mRNA, path_Q_exon, path_Q_CDS, path_Q_introns_filtered
        )) {
            # i <- "k2"
            # j <- "below_05"
            # k <- path_Q_locus
