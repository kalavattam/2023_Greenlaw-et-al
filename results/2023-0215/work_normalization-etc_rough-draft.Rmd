...
#NOTE 2/2 variables.xlsx is on your system
...
#  Add Alison's original samples names to the 'samples' data frame using the
#+ 't_xl' data frame
t_xl <- t_xl %>%
...
stably_expressed_SC <- tbl$stably_expressed[!is.na(tbl$stably_expressed)]
...
## Run analyses with *K. lactis* features only
### Perform size-factor estimation
Here, we subset out the *K. lactis* features and are thus using only
*S. cerevisiae* features in the size-factor estimation. No control genes are
used.
...
## Run analyses with all *K. lactis* features as `controlGenes`
### Perform normalization
To run analyses using all *K.lactis* features as `controlGenes`, we use a
logical vector (vector composed of elements with values of either `TRUE` or
`FALSE`) obtained from parsing the `rowRanges` data frame within the `dds`
object. In essense, we're saying, "Return `TRUE` if the `rowRanges` variable
`genome` has a value of `K_lactis`; otherwise, return `FALSE`." `BiocGenerics::estimateSizeFactors()` is using the values associated with those
`TRUE`s to isolate the counts for *K. lactis*-specific features, then in turn
using those values to calculate the size factors.
...
stably_expressed_SC.ctrl_KL <- tbl$stably_expressed[!is.na(tbl$stably_expressed)]
length(stably_expressed_SC.ctrl_KL)
...
## Run analyses with stable *K. lactis* features as `controlGenes`
### Perform normalization
to calculate the size factors using stable *K.lactis* features as
`controlGenes`, we use the stably expressed features determined earlier and
saved in variable `stably_expressed_KL`.
...
stably_expressed_SC.ctrl_KL_stab <- tbl$stably_expressed[!is.na(tbl$stably_expressed)]
length(stably_expressed_SC.ctrl_KL)
...
title <- "Size factors est. w/stably expressed KL"
subtitle <- "Points: SC; left: up G1; right: up Q."
