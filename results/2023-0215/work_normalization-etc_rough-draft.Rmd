...
dds <- DESeq2::DESeqDataSetFromMatrix(
    ...
    colData = col_data,
    # design = ~ state + transcription + state:transcription,  
    #TODO Try out/mess around with complicated models later
...
*(We may want to exclude the K. lactis data prior to doing this&mdash;not*
*sure...)*
```{r}
rlog_F <- DESeq2::rlog(dds, blind = FALSE) %>%
    SummarizedExperiment::assay() %>%
    tibble::as_tibble() %>%
    dplyr::bind_cols(., tibble::as_tibble(dds@rowRanges))

rlog_T <- DESeq2::rlog(dds, blind = TRUE) %>%
    SummarizedExperiment::assay() %>%
    tibble::as_tibble() %>%
    dplyr::bind_cols(., tibble::as_tibble(dds@rowRanges))
